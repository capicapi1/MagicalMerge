(()=>{
"use strict";
const STORAGE_KEY="mahouMergeSave_v8";
...
function spawnWave(){
  ...
  cols.forEach((col,i)=>{
    ...
    state.enemies.push({id:`e${Date.now()}-${i}-${Math.random()}`,hp,maxHp:hp,boss,emoji:boss?'👑':'👾',sprite:enemySpriteFor(col),row:0,col});
  });
  renderEnemies();updateHud();
}
function renderEnemies(){const lane=$('enemy-lane');lane.innerHTML='';state.enemies.forEach(e=>{const wrap=document.createElement('div');wrap.className=`enemy-wrap${e.boss?' boss':''}`;wrap.dataset.enemyId=e.id;wrap.style.gridRow=e.row+1;wrap.style.gridColumn=e.col+1;const pct=clamp(e.hp/e.maxHp*100,0,100);wrap.innerHTML=`<div class="enemy-hp"><div style="width:${pct}%"></div></div><div class="enemy-hp-text">${Math.max(0,Math.ceil(e.hp))}/${Math.ceil(e.maxHp)}</div><div class="enemy"><img class="sprite" src="${e.sprite||enemySpriteFor(e.col||0)}" alt=""></div>`;lane.appendChild(wrap)})}
...
function deployReserveUnits(){
  ...
  for(let i=0;i<amount;i++)state.board[empty[i]]={level:1,element:'🌱',sprite:girlSpriteFor(1)};
...
function addAlly(reason,placeNow=true){
  ...
  if(placeNow&&canAddAlly()){
    ...
    state.board[index]={level:1,element:'🌱',sprite:girlSpriteFor(1)};
...
function collectPickup(){if(!state?.pickup||state.resolving)return;const {type,index}=state.pickup;if(type==='chest'){const gems=5+Math.floor(Math.random()*6);save.gems+=gems;grantXp(12);log(`Chest opened! +${gems} gems.`)}else{state.board[index]={level:1,element:'🌱',sprite:girlSpriteFor(1)};save.collectedUnits=(save.collectedUnits||0)+1;log('New Magical Girl added to the field!')}state.pickup=null;persist();renderBoard();updateHud()}
function updateHud(){if(!state)return;$('wave-label').textContent=`WAVE ${state.wave}/${state.data.waves}`;$('turn-label').textContent=`TURN ${state.turn}`;$('base-hp-bar').style.width=`${clamp(state.baseHp/state.maxBaseHp*100,0,100)}%`; if($('base-hp-text'))$('base-hp-text').textContent=`${Math.ceil(state.baseHp)} / ${Math.ceil(state.maxBaseHp)}`; $('gems-label').textContent=save.gems;$('account-label').textContent=save.accountLevel}
...
document.querySelectorAll('[data-screen]').forEach(btn=>btn.addEventListener('click',e=>{e.preventDefault();const id=btn.dataset.screen;if(id==='game-screen'&&!state){newRun(save.stage);return}showScreen(id);document.querySelectorAll('.bottom-nav').forEach(nav=>nav.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.screen===id)));if(id==='base-screen')updateBase();if(id==='girls-screen'&&$('girls-count'))$('girls-count').textContent=save.collectedUnits||0;if(id==='collection-screen'&&$('collection-count'))$('collection-count').textContent=save.collectedUnits||0;if(id==='shop-screen'&&$('shop-gems'))$('shop-gems').textContent=save.gems;}));
