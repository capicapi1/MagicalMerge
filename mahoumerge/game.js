(() => {
  ...
  const STORAGE_KEY = "mahouMergeSave_v3";
  const MAX_STAGE = 100;
  ...
  function setupNavigation(){
    document.querySelectorAll(".bottom-nav button[data-screen]").forEach(btn=>{
      btn.addEventListener("click",()=>{
        const target=btn.dataset.screen;
        if(target==="game-screen" && !state){ newRun(save.stage); return; }
        if(target==="game-screen"){ showScreen("game-screen"); return; }
        if(target==="base-screen") updateBase();
        showScreen(target);
      });
    });
  }
